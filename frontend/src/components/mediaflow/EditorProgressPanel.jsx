...
import UploadFrequencyChart from './UploadFrequencyChart';
 ...
 * embedded=true → section inside admin dashboard (no full-page chrome)
 */
export default function EditorProgressPanel({ embedded = false }) {
    ...
    async function load() {
      ...
      try {
        ...
        const params = isAdmin
          ? { editorId: selectedEditorId, ...rangeParams }
          : rangeParams;
  ...
  const subtitle = isAdmin
    ...
    : `Your upload stats · ${periodPhrase}.`;

  const chartSubtitle = useMemo(() => {
    if (!isAdmin) return `Your uploads · ${periodPhrase}`;
    const who =
      viewMode === 'all'
        ? 'All editors'
  ...
  const header = (
    <div className="mb-5 flex flex-col gap-4 sm:mb-6 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        {embedded ? (
          <>
            <h2 className="mf-text-section">Editor Progress</h2>
            <p className="mf-text-body mt-1 text-sm sm:text-[15px]">{subtitle}</p>
          </>
        ) : (
          <>
  ...
  const body = (
    <>
      ...
      {loading && !metrics ? (
        <div className="mf-text-body py-12 text-center">Loading progress...</div>
      ) : error ? (
  ...
  if (embedded) {
    return (
      <section className="min-w-0 border-t border-outline-variant pt-8">
        {header}
        {body}
      </section>
    );
